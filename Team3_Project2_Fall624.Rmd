...
##Plot of the regression of vs number of components used

validationplot(pcrTune, val.type="RMSE")

##Plot of R^2 vs number of components used

validationplot(pcrTune, val.type = "R2")

# When doing prc it is important to keep in mind that the main goal is to see  a low cross validation error with a lower number of components than the number of variables in your dataset. In these results, it is clear that the number of components are too high for easy interpretation, resulting in no dimensionality reduction. 
...
```{r rule-tree}
library(RWeka)
# create data frame with brand code as numeric
ph_pred_train_num <- ph_pred_train %>% 
  mutate(Brand.Code = as.numeric(as.factor(Brand.Code)))
set.seed(100)
ruleTune <- M5Rules(PH ~ ., data = mutate(ph_pred_train_num, PH = ph_resp_train$PH))
rulePred <- predict(ruleTune, newdata = ph_pred_test)
rulePerf <- postResample(pred = rulePred, obs = ph_resp_test$PH)
```
...
set.seed(100)
rfTune <- train(x = ph_pred_train_num, y = ph_resp_train$PH,
...
library(gbm)
set.seed(100)
boostTune <- train(x = ph_pred_train_num, y = ph_resp_train$PH,
...
library(Cubist)
set.seed(100)
cubistTune <- train(x = ph_pred_train_num, y = ph_resp_train$PH,
